...
public class VideoPlaySimple extends Activity implements View.OnClickListener,View.OnTouchListener,MessageQueue.IdleHandler,OnErrorListener,OnPreparedListener,OnBufferingUpdateListener,OnCompletionListener/*,GestureDetector.OnDoubleTapListener*/ {
	...
	public boolean queueIdle(){
		switch(hasinit){
			case 0://初始化界面
				setContentView(R.layout.activity_video_play_simple);
...
/*	void entertempdownplay(Map<String,String>head){
	...
	void retryplay(){ hasinit=0;Looper.myQueue().addIdleHandler(this); }
	View preload; TextView playdbg; TextView scltip;
			...
			@Override public boolean onScroll(MotionEvent et1,MotionEvent et2,float x,float y){
				if(scrolltype==0) //只在第一次滚动时触发判断是何种滚动
					if(Math.abs(x)>Math.abs(y)){// 左右移动，调整进度
						...
						scrollval=20*getbright()/17;
						scrolltype=2;
					}else{ //屏在幕右侧移动，调整音量
						scrollval=20*volume*15;
						scrolltype=3;
					}
				visible(scltip,true);
				switch(scrolltype){
					case 1:
						scrollval-=x;
						sclreal=(int)Math.floor(scrollval/15);
						//int curpos=...;
						sclfix(15,0-(vp.getCurrentPosition()/1000),(vp.getDuration()-vp.getCurrentPosition())/1000);
	...
	int buffprog=0;
	void progressupdate(){
		pb.setProgress((vp.getCurrentPosition()*1000)/vp.getDuration());
		pb.setSecondaryProgress(buffprog*10);
