...
public class GetAllowance extends Service implements DialogInterface.OnClickListener{
	...
	GetAllowance This=this;
	boolean gotinfo=false;
	int tvs=0,raffs=0,golds=0;
	String cookie="",name="",initStr="{\"roomid\":295460,\"uid\":7354572585223}",errmsg="";
	...
	void log(String s){
		...
		//生成通知(); 
		if(s.length()<20) //阻止调试信息
		init.obtainMessage(9,s).sendToTarget();
	}
	public void joinTV(final int real_roomid,final String tv_id){//仅限由handler进入
		new AsyncTask<Void,Void,Integer>(){
			@Override protected Integer doInBackground(Void[]p){
				String s=网络.获得数据("GET", "http://api.live.bilibili.com/gift/v2/smalltv/join?roomid="+real_roomid+"&raffleId="+tv_id, cookie, "");
				FSON j=new FSON(s);
				if(j.canRead()) return j.get("code",5001);
				return 408;
			}
			@Override protected void onPostExecute(Integer s){
				switch(s){
					case 0: case -400: //0=加入成功；-400=已进入抽奖
						tvs++; onJoinSuccessful(0); break;
					case -401://登录信息过期
						onLogout();break;
					case 408://网络超时
	...
	public void joinRaff(final int real_roomid){//仅限由handler进入
		new AsyncTask<Void,Void,Integer>(){
			@Override protected Integer doInBackground(Void[]p){
				String s=网络.获得数据("GET", "http://api.live.bilibili.com/activity/v1/Raffle/check?roomid="+real_roomid, cookie, "");
				FSON j=new FSON(s);
				if(j.canRead())
					switch(j.get("code",5001)){//判断第一次返回的状态码，比如登录凭证过期等
						case 0://正常
							try{//获取抽奖id时可能会报空指针？
								s=j.getList("data").getObject(0).get("raffleId","-1");//RaffleId
								s=网络.获得数据("GET", "http://api.live.bilibili.com/activity/v1/Raffle/join?roomid="+real_roomid+"&raffleId="+s, cookie, "");
								j=new FSON(s);
								if(j.canRead()) return j.get("code",5001);
							}catch(Exception e){}
							return 5002;
						default:
							return j.get("code",5001);
					}
				return 4081;
			}
			@Override protected void onPostExecute(Integer s){
				switch(s){
					case 0: case -400: //0=加入成功；-400=已进入抽奖
						raffs++; onJoinSuccessful(1); break;
					case -401://登录信息过期
						onLogout();break;
					case 4081://网络超时
	...
	public void onClick(DialogInterface di, int index) {
		switch(index){
			...
			case 2:checklog();/*confirmSignOut();*/break;//todo
//			case 3:随机引用();break;
